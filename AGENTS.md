...
TaskPilotAgent is a sophisticated AI agent orchestration framework built with Python and FastAPI. It implements a multi-agent system that can plan, execute, and summarize complex tasks using various LLM providers (OpenAI, Codex, Gemini) and extensible MCP (Model Context Protocol) tools.
...
llm:
  provider: "openai"                # openai | Codex | gemini
  config:
    api_key: "sk-xxx"
