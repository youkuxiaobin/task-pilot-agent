...
| 消息渠道 | `message_send` | 向本地任务流或配置的外部 webhook 发送消息 | 低到高 |
| 记忆 | `memory_search`、`memory_add`、`memory_delete` | 显式检索或维护长期记忆 | 中 |
...
- 本地 MCP 已注册文件局部替换、glob、grep、长进程管理、浏览器 Agent、图片生成、技能、配置、MCP 管理、消息发送和记忆工具。
