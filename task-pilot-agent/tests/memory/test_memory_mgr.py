...
class TestMemoryManager:
    ...
    def memory_manager(self, mock_settings):
        ...
        with patch('meta_agent.memory.memory_mgr.Memory') as mock_memory_class:
            mock_memory = Mock()
            mock_memory_class.return_value = mock_memory
            
            with patch('meta_agent.memory.memory_mgr.PlanManager') as mock_plan_manager_class:
                mock_plan_manager = Mock()
                mock_plan_manager_class.return_value = mock_plan_manager
                
                with patch('meta_agent.memory.memory_mgr.RAGRetriever') as mock_rag_class:
                    mock_rag = Mock()
    ...
    def test_initialization_default_settings(self):
        ...
        with patch('meta_agent.memory.memory_mgr.AgentSettings') as mock_settings_class:
            mock_settings = Mock()
            mock_settings.memory = Mock()
            mock_settings.memory.search_memory = True
            mock_settings.memory.search_rag = True
