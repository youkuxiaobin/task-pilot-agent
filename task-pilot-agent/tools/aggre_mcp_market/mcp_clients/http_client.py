...
def _import_streamable_http_components():
    try:
...
class StreamableHttpMCPClient(MCPClientBase):
    ...
    def list_tools(self) -> List[ToolInfo]:
        async def _run() -> List[ToolInfo]:
            streamablehttp_client, ClientSession = _import_streamable_http_components()
            headers = self._headers()
            client_kwargs: Dict[str, Any] = {}
            if headers:
                client_kwargs["headers"] = headers

            async with streamablehttp_client(self.url, **client_kwargs) as (read, write, _):
